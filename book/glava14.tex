...
\index{Метаобъект}метаобъекта. Этот метаобъект создаётся для каждого потомка \Sys{QObject} и сохраняет
...
\begin{itemize}
\item имеют имя (\Sys{QObject :: objectName ()}), которое используется в \Sys{Qt} для реализации различных
возможностей (стили, \Sys{QML} и т.д.);
\item могут занимать место в иерархии других объектов \Sys{QObject};
\item могут иметь сигнально-слотовые соединения с другими объектами \Sys{QObject}.
\end{itemize}
...
классов с использованием мастера описано в разделе~\ref{ch13:1}). Назовём новый класс \Sys{IconizedLineEdit}. После создания
...
растянуть{\textbackslash}сжать изображения мы передаём \Sys{true} методу метки\Sys{ setScaledContents}(). Это
...
Добавим к файлу \Sys{iconizedlineedit.cpp} реализацию для этих методов.
...
Пути к файлам изображений для значков мы передаём в конструктор \index{Класс!QPixmap}\Sys{QPixmap}. Изображения
...
меню \Sys{Analyze->Valgrind Memory Analyzer} (\emph{Анализ->Анализатор памяти Valgrind}). Начнётся анализ работы программы и программа запустится. Затем закройте окно программы. После
...
переопределив метод  \Sys{QObject::event()}
 ...
 \item приведите объект события до нужного вам класса событий;
 \item выполните обработку или вызовите метод, который её выполнит, верните значение \Sys{true}, которое означает, что событие
 ...
 Верните из метода значение, которое вернёт метод \Sys{event()} базового класса после выполнения.
\end{itemize}
...
Обычно метод для получения значения называют так же как и свойство, а метод для установки значения ещё и имеет префикс
...
\item Добавьте возможность проверки правильности введённых данных с помощью класса \Sys{QValidator}. 
