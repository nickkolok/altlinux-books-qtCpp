...
В этом параграфе мы вернемся к рассмотренным ранее \emph{операторам} \Sys{cin} и
...
\footnotetext{\Sys{cout.width(n)} устанавливает ширину поля вывода, подробнее об этом в п.~\ref{ch08:1.2}}
...
&\ \linebreak\ \linebreak\ \linebreak\Sys{r=\ \ \ \ \ 253}\\\hline
...
\lstinline!cout<<h<<endl;!&\ \linebreak\ \linebreak\ \linebreak\Sys{h=1234.658}\\\hline
\Sys{dec} &\raggedright Перевод числа в десятичную систему (по умолчанию) &
\lstinline!int r=0253;!\linebreak
\lstinline!cout<<"r="<<dec<<r<<endl;!&\ \linebreak\ \linebreak\Sys{r=171}\\\hline
...
\item Отрыть файл с помощью функции \Sys{open}.
...
\emph{Открыть файл} (качестве примера возьмем файл \Sys{abc.txt}) в \emph{режиме
...
\item Отрыть файл с помощью функции \Sys{fopen}.
...
Рассмотрим пример чтения данных из файла. Пусть в файле \Sys{test.txt} хранится следующая информация:
\begin{verbatim}
