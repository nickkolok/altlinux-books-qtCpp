...
окне мастера нужно вибрать раздел \Sys{Other Project} (Другой проект) и тип 
...
\index{Переменные qmake!HEADERS}\Sys{HEADERS} &Заголовочные файли программы~\Sys{*.h}.&
...
Этот инструмент \Sys{Qt} несет ответственность за весь процесс компиляции проекта. Он читает содержание
...
с параметром -{-}\Sys{project}. Этим приемом удобно
...
(настройки размещения для нее --- в поле \Sys{Build directory}). Это позволяет построить и
хранить одновременно несколько вариантов построенного проекта для различных инструментариев. Также это сохраняет папку
с исходным кодом от засорения файлами, созданных в процессе построения проекта. При выключенном \Sys{Shadow
build} промежуточные файлы и папка с построенной программе сохраняются в папке, которая содержит файл
проекта.
...
\index{Функция!qDebug}\Sys{qDebug()}. Рассмотрим пример ее использования:
\begin{lstlisting}
#include <QDebug>
...
методом \Sys{isNull()}, который возвращает значение \Sys{true} только для еще не
...
Последние две строки возможны за счет неявного преобразования \Sys{char*} в
\Sys{QString}. Класс \Sys{QStringList}, которого мы коснулись в примерах
предыдущего раздела посвященного текстовым строкам, наследует от \Sys{QList{<}QString{>}}
...
\hline \hline
\endhead
\index{Класс!QList}\Sys{QList} & Список общего назначенния для использования в большинстве ситуаций, которые возникают при 
...
\index{Класс!QHash}\Sys{QHash} & Контейнерний класс для хеша. Элементы добавляют парами: ключ --- значение. Элементы хранятся в хеше в произвольном порядке. Позволяет выполнять очень быстрый поиск  элемента по ключу.\\\hline
\index{Класс!QMultiHash}\Sys{QMultiHash} & Контейнерний класс хеша, оздан для удобной работы с тем, чтобы каждому ключу соответствовало несколько значений. Метод \Sys{insert()} не заменяет значение ключа, если ключ уже
...
чтения одной строки текстового файла используют методы \Sys{canReadLine()} и \Sys{readLine()}.
...
\index{Класс!QTextStream}\Sys{QTextStream} для получения введенной пользователем
...
предоставляемых \Sys{Qt}. Используйте классы QFile, QTextStream для записи. Адрес текстового файла жестко задайте в тексте
программы. 
